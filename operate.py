...
class Operate:
    def __init__(self, args):
        ...
        #
        self.segmenter_output = np.zeros([240,320], dtype=np.uint8)
        ...
        if args.ckpt == "":
            ...
            self.segmenter = Segmenter(args.ckpt, use_gpu=False, model=args.model)
    ...
    def detect_fruit(self):
        if self.command['inference'] and self.segmenter is not None:
            self.segmenter_output, self.network_vis = self.segmenter.segment_single_image(self.img,
			 resize_to = (192, 256), labels=['bg', 'apple', 'banana', 'pear', 'lemon'])
            self.command['inference'] = False
            self.file_output = (self.segmenter_output, self.ekf)
            self.notification = f'{len(np.unique(self.segmenter_output))-1} fruit type(s) found'
...
if __name__ == "__main__":
    ...
    while not start:
        ...
        x_ = min(counter, 600)
        if x_ < 600:
            canvas.blit(pibot_animate[counter%10//2], (x_, 530))
            pygame.display.update()
            counter += 1
